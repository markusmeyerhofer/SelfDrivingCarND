...
void Vehicle::update_state(map<int, vector <vector<int>>> predictions) {
    ...
    */
    ...
    vector<string> states = getFeasibleStates();

    double speedFactor = 1.0;
    double laneFactor = 15.0;
    ...
    vector<double> costs;
        ...
        for (string test_state : states) {
            ...
            cout << "### Prediction 1 second ###" << endl;
            cout << "actual state: " << state<< " tested state: " << test_state << endl;
            ...
            int test_lane = pred_lane;
            if (test_state == "LCL" || test_state == "PLCL") {
                if (pred_lane < 3) test_lane = pred_lane + 1;
            }
            if (test_state == "LCR" || test_state == "PLCR") {
                if (pred_lane > 0) test_lane = pred_lane - 1;
            }

            cost += speedFactor*(target_speed - pred_v);
            cost += laneFactor*(pow(goal_lane - test_lane, 2));
            cout << " pred_lane " << pred_lane << " test_lane " << test_lane << endl;
            if (pred_lane < 0 || pred_lane > 3) {
                cost += 1000;
            }

            /*
            cout << "### Prediction 2 seconds ###" << endl;

            test_v_state = test_v.state_at(2);
            pred_lane = test_v_state[0];
            pred_s = test_v_state[1];
            pred_v = test_v_state[2];
            pred_a = test_v_state[3];
            //cout << "pred lane: " << pred_lane << " s: " << pred_s << " v: " << pred_v << " a: " << pred_a << endl;
            ...
            cout << "### Prediction 3 seconds ###" << endl;

            test_v_state = test_v.state_at(3);
            pred_lane = test_v_state[0];
            pred_s = test_v_state[1];
            pred_v = test_v_state[2];
            pred_a = test_v_state[3];
    ...
    cout << "current state: " << state << " in lane " << lane << endl;
}

vector <string> Vehicle::getFeasibleStates() {
    vector<string> feasibleStates;
    if (state == "KL") {
        feasibleStates = {"KL", "PLCL", "PLCR"};
    }
    if (state == "PLCL") {
        feasibleStates = {"KL", "LCL", "PLCL"};
    }
    if (state == "PLCR") {
        feasibleStates = {"KL", "LCR", "PLCR"};
    }
    if (state == "LCL") {
        feasibleStates = {"KL", "LCL"};
    }
    if (state == "LCR") {
        feasibleStates = {"KL","LCR"};
    }
